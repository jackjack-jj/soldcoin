...
bool AppInit2(int argc, char* argv[])
{
    ...
    //
    if (fDaemon)
        fprintf(stdout, "solidcoin server starting\n");
    strErrors = "";
    int64 nStart;
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors += _("Error loading wallet.dat: Wallet requires newer version of SolidCoin      \n");
        else
    ...
    fGenerateSolidCoins = GetBoolArg("-gen");
    ...
    /*  //dont have preset transaction fees
    if (mapArgs.count("-paytxfee"))
    {
        if (!ParseMoney(mapArgs["-paytxfee"], nTransactionFee))
        {
            wxMessageBox(_("Invalid amount for -paytxfee=<amount>"), "SolidCoin");
            return false;
        }
        if (nTransactionFee > 0.25 * COIN)
            wxMessageBox(_("Warning: -paytxfee is set very high.  This is the transaction fee you will pay if you send a transaction."), "SolidCoin", wxOK | wxICON_EXCLAMATION);
    }
    */
