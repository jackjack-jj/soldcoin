...
void CMainFrame::OnPaintListCtrl(wxPaintEvent& event)
{
    ...
    if (fGenerateSolidCoins)
        strGen = _("    Generating");
    if (fGenerateSolidCoins && vNodes.empty())
    ...
    string strDefaultAddress = CSolidCoinAddress(pwalletMain->vchDefaultKey).ToString();
...
{
    event.Check(fGenerateSolidCoins);
}
...
void CMainFrame::OnButtonNew(wxCommandEvent& event)
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_vMasterKey)
    {
        ...
        // Generate new key
        _REGEN:
        strAddress = CSolidCoinAddress(pwalletMain->GetOrReuseKeyFromPool()).ToString();
        if(strAddress[0]!='s')  goto _REGEN;    //ensure we have an 's'
...
void CSendDialog::OnButtonSend(wxCommandEvent& event)
{
    ...
    TRY_CRITICAL_BLOCK(cs_sendlock)
    {
        ...
        if(strAddress[0]!='s')
        {
            wxMessageBox(_("That address supplied doesn't appear to be a SolidCoin address  "), _("Send Coins"));
            return;
        }
        ...
        if (nValue + MIN_TX_FEE > pwalletMain->GetBalance())
        {
        ...
        CSolidCoinAddress address(strAddress);
        bool fSolidCoinAddress = address.IsValid();

        if (fSolidCoinAddress)
        {
...
void CSendingDialog::StartTransfer()
{
    ...
    if (nPrice + MIN_TX_FEE > pwalletMain->GetBalance())
    {
...
void CMyTaskBarIcon::Show(bool fShow)
{
    ...
    if (fShow)
    {
        string strTooltip = _("SolidCoin");
        if (fGenerateSolidCoins)
            strTooltip = _("SolidCoin - Generating");
        if (fGenerateSolidCoins && vNodes.empty())
            strTooltip = _("SolidCoin - (not connected)");
...
#else
            SetIcon(solidcoin80_xpm, strTooltip);
#endif
        }
    }
    else
