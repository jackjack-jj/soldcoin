...
int64 AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 18900000.0)
...
int64 AmountFromValueStr(const Value& value)
{
    ...
    if (!MoneyRange(nAmount))   throw JSONRPCError(-3, "Invalid amount");
    return nAmount;
}
...
Value getgenerate(const Array& params, bool fHelp)
{
    ...
    return (bool)fGenerateSolidCoins;
}
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateSolidCoins(fGenerate, pwalletMain);
    return Value::null;
}
...
Value real_getinfo(bool bSC, const Array& params, bool fHelp)
{
    Object obj;
    ...
    obj.push_back(Pair("generate",      (bool)fGenerateSolidCoins));
    ...
    if(bSC)
    {
        obj.push_back(Pair("int_balance",   ValueFromAmountStr(pwalletMain->GetBalance())));
        obj.push_back(Pair("int_fee",       ValueFromAmountStr(MIN_TX_FEE)));
    }
    return obj;
}
...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    return real_getinfo(false,params, fHelp);
}
...
Value sc_getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "sc_getinfo\n"
            "Returns an object containing various state info.");
    return real_getinfo(true,params, fHelp);
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    CSolidCoinAddress address(pwalletMain->GetOrReuseKeyFromPool());
...
CSolidCoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapAddressBook)
    {
        ...
        if (account.vchPubKey.empty() || bForceNew || bKeyUsed)
        {
            ...
            else
            {
                ...
                pwalletMain->SetAddressBookName(CSolidCoinAddress(account.vchPubKey), strAccount);
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid solidcoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapAddressBook)
    {
        map<CSolidCoinAddress, string>::iterator mi = pwalletMain->mapAddressBook.find(address);
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapAddressBook)
    {
        BOOST_FOREACH(const PAIRTYPE(CSolidCoinAddress, string)& item, pwalletMain->mapAddressBook)
        {
...
Value gettxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 0)
        throw runtime_error(
            "gettxfee\n"
            "Returns the current fixed transaction fee");
...
Value real_sendtoaddress(bool bSC, const Array& params, bool fHelp)
{
    ...
    // Amount
    int64 nAmount=0;
    if(bSC) nAmount = params[1].get_int64();
    else    nAmount = AmountFromValue(params[1]);
    ...
    CRITICAL_BLOCK(pwalletMain->cs_vMasterKey)
    {
        ...
        string strError = pwalletMain->SendMoneyToSolidCoinAddress(address, nAmount, wtx);
        if (strError != "")
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <solidcoinaddress> <amount> [comment] [comment-to]\n"
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <solidcoinaddress> <amount> [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001");
    return real_sendtoaddress(false, params,fHelp);
}
...
Value sc_sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sc_sendtoaddress <solidcoinaddress> <amount> [comment] [comment-to]\n"
            "<amount> is an integer from 0 to 18900000000000\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "ssc_endtoaddress <solidcoinaddress> <amount> [comment] [comment-to]\n"
            "<amount> is an integer from 0 to 18900000000000");
    return real_sendtoaddress(true, params,fHelp);
}



Value real_getreceivedbyaddress(bool bSC,const Array& params, bool fHelp)
{
    // SolidCoin address
    CSolidCoinAddress address = CSolidCoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid solidcoin address");
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    return real_getreceivedbyaddress(false,params,fHelp);
}
...
Value sc_getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "sc_getreceivedbyaddress <solidcoinaddress> [minconf=1]\n"
            "Returns the total amount received by <solidcoinaddress> in transactions with at least [minconf] confirmations.");

    return real_getreceivedbyaddress(true,params,fHelp);
}


void GetAccountAddresses(string strAccount, set<CSolidCoinAddress>& setAddress)
{
    CRITICAL_BLOCK(pwalletMain->cs_mapAddressBook)
    {
        BOOST_FOREACH(const PAIRTYPE(CSolidCoinAddress, string)& item, pwalletMain->mapAddressBook)
...
Value real_getbalance(bool bSC, const Array& params, bool fHelp)
{
    if (params.size() == 0)
    {
        if(bSC) return  ValueFromAmountStr(pwalletMain->GetBalance());
        else    return  ValueFromAmount(pwalletMain->GetBalance());
    }
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    return real_getbalance(false,params,fHelp);
}
...
Value sc_getbalance(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
    {
        throw runtime_error("sc_getbalance [account] [minconf=1]\n"
            "If [account] is not specified, returns the server's total available balance.\n"
            "If [account] is specified, returns the balance in the account.");
    }
    return real_getbalance(true,params,fHelp);
}



Value real_movecmd(bool bSC, const Array& params, bool fHelp)
{
    string strFrom = AccountFromValue(params[0]);
    string strTo = AccountFromValue(params[1]);
    int64 nAmount =0;
    if(bSC) nAmount = params[2].get_int64();
    else    nAmount = AmountFromValue(params[2]);
    if (params.size() > 3)
...
Value movecmd(const Array& params, bool fHelp)
{
    ...
    return real_movecmd(false,params,fHelp);
}
...
Value sc_movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            "sc_move <fromaccount> <toaccount> <amount> [minconf=1] [comment]\n"
            "Move from one account in your wallet to another.");
    return real_movecmd(true,params,fHelp);
}

Value real_sendfrom(bool bSC, const Array& params, bool fHelp)
{
    string strAccount = AccountFromValue(params[0]);
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    return real_sendfrom(false,params,fHelp);
}
...
Value sc_sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sc_sendfrom <fromaccount> <tosolidcoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is an integer 0 to 18900000000000\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sc_sendfrom <fromaccount> <tosolidcoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
    ...
    return real_sendfrom(true,params,fHelp);
}


Value real_sendmany(bool bSC, const Array& params, bool fHelp)
{
    string strAccount = AccountFromValue(params[0]);
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid solidcoin address:")+s.name_);
        ...
        scriptPubKey.SetSolidCoinAddress(address);
        int64 nAmount = 0;
        if(bSC) AmountFromValueStr(s.value_);
        else    AmountFromValue(s.value_);
        totalAmount += nAmount;
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendmany <fromaccount> {address:amount,...} [minconf=1] [comment]\n"
            "amounts are double-precision floating point numbers");
    return real_sendmany(false, params,fHelp);
}
...
Value sc_sendmany(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sc_sendmany <fromaccount> {address:amount,...} [minconf=1] [comment]\n"
            "amounts are an integer 0 to 18900000000000\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sc_sendmany <fromaccount> {address:amount,...} [minconf=1] [comment]\n"
            "amounts are an integer 0 to 18900000000000");
    return real_sendmany(true, params,fHelp);
}
...
Value ListReceived(bool bSC, const Array& params, bool fByAccounts)
{
    // Minimum confirmations
    int nMinDepth = 1;
    if (params.size() > 0)
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    return ListReceived(false, params, false);
}
...
Value sc_listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            "sc_listreceivedbyaddress [minconf=1] [includeempty=false]\n"
            "[minconf] is the minimum number of confirmations before payments are included.\n"
            "[includeempty] whether to include addresses that haven't received any payments.\n"
            "Returns an array of objects containing:\n"
            "  \"address\" : receiving address\n"
    ...
    return ListReceived(true, params, false);
}
...
Value listreceivedbyaccount(const Array& params, bool fHelp)
{
    ...
    return ListReceived(false, params, true);
}
...
Value sc_listreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            "sc_listreceivedbyaccount [minconf=1] [includeempty=false]\n"
            "[minconf] is the minimum number of confirmations before payments are included.\n"
            "[includeempty] whether to include accounts that haven't received any payments.\n"
            "Returns an array of objects containing:\n"
            "  \"account\" : the account of the receiving addresses\n"
    ...
    return ListReceived(true, params, true);
}

void ListTransactions(bool bSC, const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
...
Value real_listtransactions(bool bSC, const Array& params, bool fHelp)
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapWallet)
    {
        ...
        for (; it != txByTime.rend(); ++it)
        {
            ...
            if (pwtx != 0)
                ListTransactions(bSC,*pwtx, strAccount, 0, true, ret);
            CAccountingEntry *const pacentry = (*it).second.second;
            if (pacentry != 0)
                AcentryToJSON(bSC,*pacentry, strAccount, ret);
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            "listtransactions [account] [count=10] [from=0]\n"
            "Returns up to [count] most recent transactions skipping the first [from] transactions for account [account].");
    return real_listtransactions(false,params,fHelp);
}
...
Value sc_listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            "sc_listtransactions [account] [count=10] [from=0]\n"
            "Returns up to [count] most recent transactions skipping the first [from] transactions for account [account].");
    return real_listtransactions(true,params,fHelp);
}


Value real_listaccounts(bool bSC,const Array& params, bool fHelp)
{
    int nMinDepth = 1;
    if (params.size() > 0)
        nMinDepth = params[0].get_int();
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapAddressBook)
    {
        BOOST_FOREACH(const PAIRTYPE(CSolidCoinAddress, string)& entry, pwalletMain->mapAddressBook) {
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    return real_listaccounts(false,params,fHelp);
}
...
Value sc_listaccounts(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "sc_listaccounts [minconf=1]\n"
            "Returns Object that has account names as keys, account balances as values.");
    return real_listaccounts(true,params,fHelp);
}

Value real_gettransaction(bool bSC, const Array& params, bool fHelp)
{
    uint256 hash;
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapWallet)
    {
        ...
        Array details;
        ListTransactions(bSC,pwalletMain->mapWallet[hash], "*", 0, false, details);
...
Value gettransaction(const Array& params, bool fHelp)
{
    ...
    return real_gettransaction(false,params,fHelp);
}
Value sc_gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "sc_gettransaction <txid>\n"
            "Get detailed information about <txid>");
    return real_gettransaction(true,params,fHelp);
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "SolidCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "SolidCoin is downloading blocks...");
...
void ThreadRPCServer2(void* parg)
{
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 8555));
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    SSLIOStreamDevice d(sslStream, fUseSSL);
    iostreams::stream<SSLIOStreamDevice> stream(d);
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "8555")))
        throw runtime_error("couldn't connect to server");
#else
    if (fUseSSL)
        throw runtime_error("-rpcssl=1, but solidcoin compiled without full openssl libraries.");

    ip::tcp::iostream stream(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "8555"));
    if (stream.fail())
        throw runtime_error("couldn't connect to server");
#endif
...
int CommandLineRPC(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if (strMethod == "sc_getreceivedbyaddress"&& n > 1) ConvertTo<boost::int64_t>(params[1]);
        if (strMethod == "sc_getbalance"          && n > 1) ConvertTo<boost::int64_t>(params[1]);
        if (strMethod == "sc_listaccounts"        && n > 0) ConvertTo<boost::int64_t>(params[0]);
        if (strMethod == "sc_listtransactions"    && n > 1) ConvertTo<boost::int64_t>(params[1]);
        if (strMethod == "sc_listtransactions"    && n > 2) ConvertTo<boost::int64_t>(params[2]);
        if (strMethod == "sc_listreceivedbyaddress"  && n > 0) ConvertTo<boost::int64_t>(params[0]);
        if (strMethod == "sc_listreceivedbyaddress"  && n > 1) ConvertTo<bool>(params[1]);
        if (strMethod == "sc_listreceivedbyaccount"  && n > 0) ConvertTo<boost::int64_t>(params[0]);
        if (strMethod == "sc_listreceivedbyaccount"  && n > 1) ConvertTo<bool>(params[1]);
        if (strMethod == "sc_sendtoaddress"          && n > 1) ConvertTo<boost::int64_t>(params[1]);
        if (strMethod == "sc_move"                   && n > 2) ConvertTo<boost::int64_t>(params[2]);
        if (strMethod == "sc_move"                   && n > 3) ConvertTo<boost::int64_t>(params[3]);
        if (strMethod == "sc_sendfrom"               && n > 2) ConvertTo<boost::int64_t>(params[2]);
        if (strMethod == "sc_sendfrom"               && n > 3) ConvertTo<boost::int64_t>(params[3]);
        if ( (strMethod == "sendmany"||strMethod == "sc_sendmany") && n > 1)
        {
        ...
        if (strMethod == "sc_sendmany"              && n > 2) ConvertTo<boost::int64_t>(params[2]);
