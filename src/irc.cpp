...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        CAddress addrConnect("193.107.204.22", 6667); // irc.lfnet.org
        if (!fTOR)
        {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #solidcoinTEST\r");
            Send(hSocket, "WHO #solidcoinTEST\r");
        } else {
            // randomly join #solidcoin00-#solidcoin99
            int channel_number = GetRandInt(5);
            if(channel_number==4) channel_number=69;        //connect to other versions at this channel
            Send(hSocket, strprintf("JOIN #solidcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #solidcoin%02d\r", channel_number).c_str());
        }
