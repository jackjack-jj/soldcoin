...
uint256 hashGenesisBlock("0x00000000c9b670fcf609a7a24adddd59317dca706417dd9efd76caf2ba44526e");
...
bool CTransaction::CheckTransaction(int nBlockHeight) const
{
    ...
    if(dwTXSize > MAX_BLOCK_SIZE)
        return error("CTransaction::CheckTransaction() : block size limits failed");

    if(nBlockHeight>28500)
    {
        if(dwTXSize>MAX_TX_SIZE)   return error("CTransaction::CheckTransaction() : tx size limits failed");
    }
...
bool CTransaction::AcceptToMemoryPool(int blockheight,CTxDB& txdb, bool fCheckInputs, bool* pfMissingInputs)
{
    if (pfMissingInputs)
        *pfMissingInputs = false;

    if (!CheckTransaction(blockheight))
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 2 years, 20 coins per hour * 24 hours per day * 365 days in a year * 2 years
...
bool CBlock::AcceptBlock()
{
    ...
    if (!fTestNet)
    {
        if  ((nHeight ==  940 && hash != uint256("0x0000000099dcf5caea5b0bfc53b6d7293b2104b0527dc3793734ab58d43b8695"))  ||     //first 30,080 coins reserved for bounties
            (nHeight ==  10000 && hash != uint256("0x00000000024b9765d4aba0b7b6ad9ba3778ad2224fde3b8151af1d934150e59e")) ||       //first checkpoint
            (nHeight ==  20000 && hash != uint256("0x0000000000032592040d2d8dc58c007e758f90c31018d35074476ab3b992e5bc"))
        )
        {
    ...
    //if(pindexPrev->nHeight==940)
    {
        //exit(0);  //used to stop mining after 30080 coins were found
    }
    ...
    if (hashBestChain == hash)
        CRITICAL_BLOCK(cs_vNodes)
            BOOST_FOREACH(CNode* pnode, vNodes)
                if (nBestHeight > (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 2000 : 0))
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "New York Times | 20/August/2011 | Once Again, Fear Sends Stocks Down - After starting out the week with hopes of newfound stability, stock market investors ended up experiencing another ugly week as worries about the global economy resurfaced.";
        ...
        block.nVersion = 1;
        block.nTime    = 1313815362;
        block.nBits    = 0x1d00ffff;
        block.nNonce   = 1006583540 ;

        if (fTestNet)
        {
            //wxMessageBox(_("ftestnetdoesn't appear to be a SolidCoin address  "), _("Send Coins"));
        ...
        {
            block.nNonce=0; //should be 0 when genning your new block
           ...
           // creating a different genesis block:
            ...
            FILE * pFile;
            pFile = fopen ("genblock.txt","w");
            fprintf(pFile,"timestamp text: %s \n",pszTimestamp);
            fprintf(pFile,"block.nTime = %u \n", block.nTime);
            fprintf(pFile,"block.nNonce = %u \n", block.nNonce);
            fprintf(pFile,"block.GetHash = %s\n", block.GetHash().ToString().c_str());
            fprintf(pFile,"hashGenesisBlock = %s\n", hashGenesisBlock.ToString().c_str());
        ...
        printf("%s\n", hashGenesisBlock.ToString().c_str());
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x1e4eff5805a429b63a91e6851416702ddc6a96cd1e121eebf18e49bd9de7fa0d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xde, 0xad, 0xba, 0xbe };
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    CRITICAL_BLOCK(cs_mapTransactions)
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            if(nTxSize>MAX_TX_SIZE) //make sure its under transaction size
                continue;
            ...
            int64 nMinFee = MIN_TX_FEE;//tx.GetMinFee(nBlockSize, fAllowFree, true);
...
void static ThreadSolidCoinMiner(void* parg);

void static SolidCoinMiner(CWallet *pwallet)
{
    printf("SolidCoinMiner started\n");
    ...
    while (fGenerateSolidCoins)
    {
        if (AffinityBugWorkaround(ThreadSolidCoinMiner))
            return;
        if (fShutdown)
            return;
        ...
        printf("Running SolidCoinMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadSolidCoinMiner(void* parg)
{
    ...
    {
        vnThreadsRunning[3]++;
        SolidCoinMiner(pwallet);
        vnThreadsRunning[3]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[3]--;
        PrintException(&e, "ThreadSolidCoinMiner()");
    } catch (...) {
        vnThreadsRunning[3]--;
        PrintException(NULL, "ThreadSolidCoinMiner()");
    }
    UIThreadCall(boost::bind(CalledSetStatusBar, "", 0));
    nHPSTimerStart = 0;
    if (vnThreadsRunning[3] == 0)
        dHashesPerSec = 0;
    printf("ThreadSolidCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[3]);
}


void GenerateSolidCoins(bool fGenerate, CWallet* pwallet)
{
    if (fGenerateSolidCoins != fGenerate)
    {
        fGenerateSolidCoins = fGenerate;
        WriteSetting("fGenerateSolidCoins", fGenerateSolidCoins);
        MainFrameRepaint();
    }
    if (fGenerateSolidCoins)
    {
        ...
        printf("Starting %d SolidCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadSolidCoinMiner, pwallet))
                printf("Error: CreateThread(ThreadSolidCoinMiner) failed\n");
            Sleep(10);
        }
    }
}
